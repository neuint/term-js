...
class ModalView extends term.TemplateEngine {
    ...
    get normalizedActions() {
        ...
        if (!actions.length)
            return [];
        return (lodashEs.isArray(actions[0]) ? actions : [actions]);
    }
    render() {
        if (this.isRendered)
            return;
...
class Modals extends term.Plugin {
    constructor() {
        super(...arguments);
        ...
        this.hideLast = () => {
            ...
            const lastModal = lodashEs.last(modalStack);
    ...
    show(options) {
        ...
        this.modalStack.push({ uuid: uuid$1, options, view });
