...
var contextMenuPlugin = require('@term-js/context-menu-plugin');
...
class Item extends term.TemplateEngine {
...
class List extends term.TemplateEngine {
...
class Dropdown extends term.Plugin {
    constructor() {
        super();
    ...
    setContextMenuPlugin() {
        ...
        if (!termInfo)
            return;
        const contextMenuPlugin$1 = termInfo.pluginManager.getPlugin(contextMenuPlugin.ContextMenu);
        if (contextMenuPlugin$1) {
            this.contextMenuPlugin = contextMenuPlugin$1;
        }
        else {
            this.contextMenuPlugin = new contextMenuPlugin.ContextMenu();
            termInfo.pluginManager.register(this.contextMenuPlugin);
        }
    }
    renderList(params) {
        const { contextMenuPlugin: contextMenuPlugin$1, container, itemsList, keyboardShortcutsManager, unlockCallback, highlightField, } = this;
        const { className = '', append } = params;
        if (!contextMenuPlugin$1 || !keyboardShortcutsManager)
            return;
        ...
        if (!unlockCallback) {
            this.unlockCallback = keyboardShortcutsManager.lock([
                NEXT_ACTION, DOWN_ACTION, UP_ACTION, SUBMIT_ACTION, contextMenuPlugin.CLOSE_ACTION,
            ]);
        }
        ...
        this.isActive = false;
        contextMenuPlugin$1.show(container, contextMenuPlugin.END_OF_LINE_TYPE, {
